...
import styles from './flow-gradient-hero-section.module.css';
...
export function LiquidGradient({ 
  ...
  return (
    <div 
      className={styles.liquidContainer}
      onMouseMove={handleMouseMove}
    >
      <div ref={containerRef} className={styles.liquidCanvasWrapper} />
      
      {(title || ctaText) && (
        <div className={styles.contentOverlay}>
          {title && (
            <h1 className={`${styles.titleMain} ${isDarkMode ? styles.dark : ''}`}>
              {title}
            </h1>
          )}
          ...
          {ctaText && (
            <button 
              className={`${styles.ctaBtn} ${isDarkMode ? styles.dark : ''}`}
              onClick={onCtaClick}
              data-cursor="CLICK"
            >
              {ctaText}
      ...
      {showPauseButton && (
        <button
          ...
          className={`${styles.pauseBtn} ${isDarkMode ? styles.dark : ''}`}
